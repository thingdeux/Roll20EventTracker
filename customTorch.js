...
var Torch = Torch || (function() {
    ...
    var version = '0.8.11',
        ...
        startFlicker = function(message) {
            var args = message.split(" ");
            var radius = parseInt(args[1],10) || 40;
            var dim_radius = parseInt(args[2],10) || (radius/2);
            var other_players = _.contains([1,'1','on','yes','true','sure','yup','-'], args[3] || 1 );
            var arc_angle = 360;

            var objs =_.chain(args)
                .rest(4)
                .uniq()
                .filter(function(a){
                    var angle=a.match(/^--(\d+)$/);
